...
public class DefaultReconnectManager extends AbsReconnectionManager{
            ...
            if (mConnectionFailedTimes > MAX_CONNECTION_FAILED_TIMES) {
                ...
                if (backupInfo != null) {
                    ...
                    if (!mConnectionManager.isConnect()) {
                        KLog.i("Prepare switch to the backup line " + backupInfo.getIp() + ":" + backupInfo.getPort() + " ...");
    ...
    private class ReconnectTestingThread extends AbsLoopThread {
        ...
        protected void runInLoopThread() throws Exception {
            if (mDetach) {
                KLog.i("ReconnectionManager already detached by framework.We decide gave up this reconnection mission!");
                shutdown();
                return;
            }

            //延迟执行
            KLog.i("Reconnect after " + mReconnectTimeDelay + " mills ...");
            ...
            KLog.i("Reconnect the server " + info.getIp() + ":" + info.getPort() + " ...");
            synchronized (mConnectionManager) {
                if (!mConnectionManager.isConnect()) {
